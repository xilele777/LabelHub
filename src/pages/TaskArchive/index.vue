  ...
  <section class="archive-page app-page">
    <header class="app-page-header">
      <div class="app-page-title">
        ...
        <a-typography-text class="app-page-desc" type="secondary">查看历史任务和已归档标注项。</a-typography-text>
      </div>
    ...
    <a-card class="archive-card">
      <a-tabs v-if="isManager" v-model:activeKey="activeTab">
    ...
    <a-modal v-model:open="taskDetailOpen" title="归档任务详情" width="680px" :footer="null">
      ...
      <a-descriptions v-if="currentTask" bordered size="small" :column="{ xs: 1, sm: 1, md: 2 }">
        ...
        <a-descriptions-item label="类型">
          <a-tag :color="getTaskTypeMeta(currentTask.type).color">{{ getTaskTypeMeta(currentTask.type).label }}</a-tag>
        </a-descriptions-item>
        <a-descriptions-item label="原状态">
          <a-tag :color="getTaskStatusMeta(currentTask.status).color">{{ getTaskStatusMeta(currentTask.status).label }}</a-tag>
        </a-descriptions-item>
        <a-descriptions-item label="负责人">{{ currentTask.owner || '-' }}</a-descriptions-item>
        <a-descriptions-item label="绑定模板">{{ currentTask.templateName || '-' }}</a-descriptions-item>
        <a-descriptions-item label="归档时间">{{ formatDate(currentTask.archivedAt) }}</a-descriptions-item>
    ...
    <a-modal v-model:open="itemDetailOpen" title="标注项详情" width="860px" :footer="null">
      <a-space v-if="currentItem" direction="vertical" size="middle" class="detail-body lh-modal-detail">
        <a-descriptions bordered size="small" :column="{ xs: 1, sm: 1, md: 2 }">
          ...
          <a-descriptions-item label="标注员">{{ currentItem.annotator || '-' }}</a-descriptions-item>
          <a-descriptions-item label="审核员">{{ currentItem.reviewer || '-' }}</a-descriptions-item>
          <a-descriptions-item label="状态">
            <a-tag :color="getDataStatusMeta(currentItem.status).color">{{ getDataStatusMeta(currentItem.status).label }}</a-tag>
          </a-descriptions-item>
          <a-descriptions-item label="归档时间">{{ formatDate(currentItem.archivedAt) }}</a-descriptions-item>
        </a-descriptions>
...
import { Button, Empty, Popconfirm, Space, Table, Tag, message, type TableColumnsType } from 'ant-design-vue';
...
const itemColumns: TableColumnsType<DataItem> = [
  ...
  { title: '归档时间', dataIndex: 'archivedAt', key: 'archivedAt', width: 148, responsive: ['xxl'] },
  { title: '操作', key: 'action', width: 156 },
];
        ...
        : h(Table<TaskItem>, {
            ...
            bodyCell: ({ column, record }: { column: { key?: string }; record: TaskItem }) => {
              ...
              if (column.key === 'action') {
                return h(Space, null, () => [
                  h(Button, { size: 'small', onClick: () => openTaskDetail(record) }, {
                    icon: () => h(EyeOutlined),
                    default: () => '查看',
                  }),
                  isManager.value
      ...
      annotationStore.archivedItems.length === 0
        ...
        : h(Table<DataItem>, {
            ...
            bodyCell: ({ column, record }: { column: { key?: string }; record: DataItem }) => {
              ...
              if (column.key === 'action') {
                return h(Space, null, () => [
                  h(Button, { size: 'small', onClick: () => openItemDetail(record) }, {
                    icon: () => h(EyeOutlined),
                    default: () => '查看',
                  }),
                  isManager.value
