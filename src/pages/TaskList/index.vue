      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'timeliness'">
            <a-tooltip>
              <template #title>
                <div>标注：{{ getTaskTimeliness(record).description }}</div>
                <div>审核：{{ getReviewTimeliness(record).description }}</div>
              </template>
              <a-space direction="vertical" :size="4">
                <a-tag class="timeliness-tag" :color="getTaskTimeliness(record).color">
                  标注 {{ getTaskTimeliness(record).label }}
                </a-tag>
                <a-tag class="timeliness-tag" :color="getReviewTimeliness(record).color">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space size="small" wrap>
              ...
              >
                <a-button type="link" size="small" danger>结束</a-button>
              </a-popconfirm>
              <a-popconfirm
