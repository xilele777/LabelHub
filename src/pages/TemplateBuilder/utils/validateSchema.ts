  ...
  data.fields.forEach((item: unknown, index: number) => {
    ...
    switch (type) {
      ...
      case FieldType.RADIO: {
        ...
        const direction = field.direction === 'horizontal' || field.direction === 'vertical'
      ...
      case FieldType.RATING: {
        const maxScore = typeof field.maxScore === 'number' && field.maxScore > 0
          ? field.maxScore : 5;
      ...
      case FieldType.SWITCH: {
        ...
        const checkedChildren = typeof field.checkedChildren === 'string' ? field.checkedChildren : undefined;
        const unCheckedChildren = typeof field.unCheckedChildren === 'string' ? field.unCheckedChildren : undefined;
        sanitizedFields.push({ ...base, type, defaultValue, checkedChildren, unCheckedChildren } as TemplateField);
        break;
      }
      case FieldType.TITLE: {
        const content = typeof field.content === 'string' ? field.content : '';
        const level = [1, 2, 3, 4, 5].includes(field.level as number)
          ? (field.level as 1 | 2 | 3 | 4 | 5) : 4;
        sanitizedFields.push({ id, type, fieldKey: '', label, content, description, level } as TemplateField);
        break;
      }
      default: {
        void type;
